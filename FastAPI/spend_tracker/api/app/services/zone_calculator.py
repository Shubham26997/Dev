...
async def recalculate_zone(
    ...
    *,
    ...
    """
    Recalculates zone for the given month/year and upserts SpendInsight.
    ...
    """
    try:
        # 1. Fetch confirmed expenses for the month
        ...
        if insight is None:
            insight = SpendInsight(id=uuid.uuid4(), month=month, year=year)
            db.add(insight)
        ...
        logger.info("Zone recalculated for %d/%d → %s (saving=%d, spend=%d)", month, year, zone, saving_score, spend_score)
        return insight
