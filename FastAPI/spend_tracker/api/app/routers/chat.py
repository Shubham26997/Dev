...
Accepts a user message + conversation history, fetches the current month's
financial context from the DB, and returns a Gemini-powered reply.
"""
import logging
...
async def chat(req: ChatRequest, db: DbDep) -> ChatResponse:
    now = datetime.now(tz=timezone.utc)
    month = req.month or now.month
    year = req.year or now.year

    # Fetch confirmed expenses for the month
    ...
    # Build multi-turn history (cap at last 10 exchanges, map "assistant" → "model")
