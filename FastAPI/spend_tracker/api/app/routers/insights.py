...
from app.models import Expense, SavingsPlan, SpendInsight
...
async def refresh_insights(db: DbDep) -> ZoneResponse:
    """On-demand: recalculate zone + call Gemini for fresh narrative. Called by 'AI Insights' button."""
    now = datetime.now(tz=timezone.utc)
    month, year = now.month, now.year

    from app.services.zone_calculator import recalculate_zone
    insight = await recalculate_zone(db, month, year, with_ai=True)
...
async def get_zone(db: DbDep) -> ZoneResponse:
    now = datetime.now(tz=timezone.utc)
    month, year = now.month, now.year
    ...
    try:
        result = await db.execute(
            select(SpendInsight)
            .where(SpendInsight.month == month, SpendInsight.year == year)
    ...
    if insight is None:
        # First visit with no data: fast calc, no Gemini
        from app.services.zone_calculator import recalculate_zone
...
async def get_savings_plan(month: int, year: int, db: DbDep) -> SavingsPlanOut | None:
    try:
        result = await db.execute(
            select(SavingsPlan)
...
async def get_spend_insight(month: int, year: int, db: DbDep) -> ZoneResponse | None:
    try:
        result = await db.execute(
            select(SpendInsight)
            .where(SpendInsight.month == month, SpendInsight.year == year)
...
async def get_monthly_history(db: DbDep) -> list[MonthlySummary]:
    """Last 12 months of spending summary for the comparison chart."""
    try:
        result = await db.execute(
            select(SpendInsight)
...
async def investment_review(month: int, year: int, db: DbDep) -> dict:
    """Gemini analysis of investment allocation and savings behaviour for the month."""
    import calendar
    ...
    try:
        ...
        # Explicitly logged "Saving" category expenses (instrument deposits)
